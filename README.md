...
- `pikabu_cookie` — Cookie залогиненной сессии Пикабу. Нужна для 18+ сообществ:
  у них полноразмерные картинки спрятаны за возрастным гейтом и анонимно не
  парсятся. Возьми значение заголовка `Cookie` из браузера (DevTools → Network →
  запрос страницы → Request Headers → Cookie) и вставь сюда. Это как пароль —
  файл `config.*.json` в `.gitignore`, наружу не уходит; со временем протухает,
  тогда вставь заново. После правки перезапусти сервис бота.
...
### Модерация через бота (общаешься с ботом в личке)

Модерируешь прямо в переписке с ботом: пишешь ему `refill` → он присылает тебе
картинки с кнопками **✅ Одобрить / ❌ Отклонить**. Пользоваться ботом может
**только владелец** (`owner_id`) — остальным «⛔ Доступ запрещён».

1. Узнай свой `user_id`: `./factory --config config.yume.json chatid`, затем
   напиши боту в личку любое сообщение — команда напечатает `user_id=...`.
   Впиши число в `owner_id` в конфиге.
2. Запусти бота: `./factory --config config.yume.json moderate-bot`.
3. Напиши боту в личку:
   - `refill` — собрать новых кандидатов, бот пришлёт карточки с кнопками;
   - `status` — статистика.
4. Жмёшь ✅/❌ под картинками. Одобренные уходят в очередь и публикуются в канал
   по одной в час; отклонённые отсеиваются. Карточки приходят в личку владельца
   (или в `moderation_chat`, если он задан).
